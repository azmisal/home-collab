    ...
    switch (reqBody.option) {
        case 1: {
            try {
                ...
                const querySnapshot = await getDocs(collection(db, 'appliances'), where('userId', '==', userId));
        ...
        case 2: {
            ...
            try {
                ...
                const q = query(collection(db, 'appliances'), where('room', '==', select), where('userId', '==', userId));
        ...
        case 3: {
            ...
            try {
                const userId = reqBody.userId;
                const q = query(collection(db, 'appliances'), where('connectionNumber', '==', select), where('userId', '==', userId));
                const querySnapshot = await getDocs(q);
                const appliancesData = querySnapshot.docs.map((doc) => doc.data());
                return NextResponse.json({ appliancesData })
        ...
        case 4: {
            ...
            try {
                const queryRef = query(collection(db, "appliances"), where("connectionNumber", "==", select), where('userId', '==', userId));
